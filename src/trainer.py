...
class TaxiBJTrainer(nn.Module):
    def __init__(self):
        ...
        # CxT×H×W
        input_shape = (1, self.temporal_frames, self.height, self.width)
        output_shape = (1, self.output_time_horizon, self.height, self.width)
    ...
    def get_trainloader(self, raw_data, shuffle=True):
        ...
        return torch.randn(16, 8, 224, 224, 1)
    ...
    def resume_train(self, ckpt_path="./taxibj_trainer.pt", resume=False):
        ...
        train_dataloader = [(torch.randn(16, 1, 8, 224, 224), torch.randn(4, 1, 8, 224, 224))]
        ...
        while epoch < self.num_epoch:
            ...
            for i, (input, target) in enumerate(train_dataloader):
                ...
                # window from utils provides already indices 
                for indices in window(
                    range(self.input_time_window),
                    self.temporal_frames,
                    self.temporal_stride,
                ...
                # (list to tensor) batch x channels x time x window x height 
                input = torch.stack(frames_seq, dim=0).to(self.device)
                target = target.to(self.device)
